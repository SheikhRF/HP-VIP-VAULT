...
import { Gauge, Zap, Route, Calendar, Car } from "lucide-react";
...
export default async function UserDetailPage({ params }: { params: { id: string } }) {
  ...
  // 2. Fetch Trips and Car details separately to ensure stability
  const { data: trips } = await supabase
    .from("trips")
    .select(`
      *,
      cars:car_id (make, model, plate)
    `)
    .eq("user_id", params.id);

  // 3. Calculate Total Mileage safely
  const totalMileage = trips?.reduce((acc: number, trip: any) => {
    return acc + (Number(trip.end_mileage || 0) - Number(trip.start_mileage || 0));
  }, 0) || 0;
        ...
        <div className="max-w-5xl mx-auto space-y-12">
          ...
          <div className="space-y-4 border-b border-white/5 pb-12">
            <h1 className="text-6xl md:text-8xl font-black italic uppercase tracking-tighter leading-none">
              {user.name.split(' ')[0]} <span className="text-orange-500 text-glow-orange">History</span>
            </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatCard icon={<Zap size={20}/>} label="Total Distance" value={`${totalMileage} miles`} />
            <StatCard icon={<Route size={20}/>} label="Total Sorties" value={user.trips?.length || 0} />
            <StatCard icon={<Car size={20}/>} label="Cars Driven" value={new Set(user.trips?.map((t: any) => t.car_id)).size} />
          </div>
                  ...
                  {user.trips?.map((trip: any) => (
                    <tr key={trip.id} className="hover:bg-white/[0.01]">
                      <td className="px-8 py-6">
                        <p className="text-xs font-black uppercase italic">{trip.cars?.make} {trip.cars?.model}</p>
                        <p className="text-[9px] font-mono text-gray-600">{trip.cars?.plate}</p>
                      </td>
                      <td className="px-8 py-6 text-xs font-bold text-orange-500 italic">
                        +{trip.end_mileage - trip.start_mileage} miles
                      </td>
